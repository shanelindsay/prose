...
| `ci-gate.yml` | `workflow_run` on "E2E Tests" | `<!-- e2e-fix-attempt:`, `<!-- agent-fix-attempt:`, `<!-- ci-gate-sha:` | Posts `/review <!-- ci-gate-sha: ... -->` comment; `<!-- e2e-fix-escalation -->` | `Codex.yml` (via `/review` comment) |
| `Codex.yml` (auto-review) | `/review` comment on PR | — | `<!-- review-verdict: clean -->` or `<!-- review-verdict: issues-found -->` | — |
...
**Sync hazard:** `run-pe-analysis.mjs` and `Codex.yml` auto-review both hardcode the same privilege-boundary path list (`src/main/**`, `src/preload/**`, `electron-builder.*`, `electron.vite.config.*`). If you update one, update the other. `validate-pipeline.sh` invariant #20 enforces this.

**Sync hazard:** `Codex.yml` emits and `pipeline-triage.yml` greps the `<!-- security-gate: true -->` sentinel. If either side drifts, the short-circuit silently breaks and privilege-boundary PRs waste API calls on scorer+PE. `validate-pipeline.sh` invariant #19 enforces this.
...
| `secrets.ANTHROPIC_API_KEY` | N/A (API auth) | N/A | Codex API calls in scorer, PE, auto-fix, auto-review |
...
- `id-token: write` is for OIDC federation only (used by `Codex-action`) — do not add it unless the job uses OIDC
- `actions: write` permission is required for `gh workflow run` dispatch calls
- When a job posts a comment that triggers another workflow, the token identity determines the `actor` filter — `github-actions[bot]` vs `Codex[bot]` vs a human username. Mismatched identity breaks `if:` conditions downstream.
...
- `run-scorer.mjs`: `Codex-sonnet-4-6`
- `run-pe-analysis.mjs`: `Codex-opus-4-6`
- `analyze-review-feedback.mjs`: `Codex-sonnet-4-6`

When Anthropic releases new model versions, update all three files. Consider using an environment variable override (e.g., `process.env.SCORER_MODEL || 'Codex-sonnet-4-6'`) to allow CI-level model pinning without code changes.

The `validate-pipeline.sh` script checks for stale model IDs — update its patterns when adding new models.

**`Codex-action@v1` model pinning.** The action does not expose a `model:` input — it delegates to the bundled Codex CLI, which the action rolls forward on every release. CLI defaults can change silently (the May 8–9 default shift to Opus 4.7 sent daily CI spend from ~$2 to $36 on identical workload). Always pass `--model <id>` via `claude_args` on every `Codex-action@v1` invocation. Current pin: `Codex-sonnet-4-6` in `Codex.yml` (auto-review + `@Codex` handler) and `pipeline-fix.yml`.
...
1. **NEVER add `id-token: write`** unless the job uses OIDC federation (currently only `Codex-action` jobs). It's not a general-purpose permission.
...
4. **NEVER edit privilege-boundary paths in only one location.** The list appears in both `Codex.yml` (auto-review prompt) and `run-pe-analysis.mjs`. Update both or neither.
...
- [ ] **`if:` condition targeting** — Conditions match the correct comment author (`github-actions[bot]`, `Codex[bot]`, or human) and sentinel.
...
- [ ] **Path list sync** — If privilege-boundary paths changed, both `Codex.yml` and `run-pe-analysis.mjs` are updated.
