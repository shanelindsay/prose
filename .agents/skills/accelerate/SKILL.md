...
Accelerate issue implementation through autonomous or semi-autonomous agent work. Covers classification, prompt generation, @Codex dispatch, and progress tracking.
...
|--------|-------|-----------|
| Labels include "spike" or "investigation" | Local (Codex) | Requires exploration |
...
| Clear acceptance criteria + bounded scope | Cloud (@Codex) | Automatable |
| Bug with repro steps | Cloud (@Codex) | Clear fix path |
| UI enhancement with clear scope | Cloud (@Codex) | Bounded feature |
...
| On @Codex comment posted | (label already applied) | Phase 3 Cloud step 2 |
| On PR created | Update to `accelerated:pr-open` | Phase 3 Cloud step 4 |
| On PR merged | Remove all `accelerated*` labels | — |

## Phase 3: Execution

### Cloud Route (@Codex on GitHub)

1. **Add `accelerated` label** to the issue
2. Post a `@Codex` comment on the issue to trigger the `Codex.yml` workflow:

```
@Codex Implement this issue.

Read AGENTS.md for project conventions (commit format, branch naming, PR format).
Create a feature branch, implement the changes, and open a PR.
```

3. The `Codex.yml` workflow handles the rest — it picks up `@Codex` mentions, creates a branch, implements, and opens a PR.
...
4. Follow standard AGENTS.md workflow (branch, commit, PR)
